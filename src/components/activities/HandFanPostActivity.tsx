...
export function HandFanPostActivity({ result, onComplete }: HandFanPostActivityProps) {
  const styles = useHandFanPostStyles();
  return (
    <ActivityMcqPostQuiz
      activityId="hand-fan"
      result={result}
...
export function HandFanDiscussion({ onComplete }: { onComplete?: () => void }) {
  const { t } = useTranslation();
  const styles = useHandFanPostStyles();
  return (
    <View style={styles.container}>
      <ActivityDiscussionCard activityId="hand-fan" styles={styles} />
          {onComplete && (
        <Button 
